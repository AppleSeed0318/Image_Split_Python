...
def split_image(image_path, coordinates, index):
    ...
    for data in coordinates:
        ...
        width = data[2]
        height = data[3]
        # Crop the frame based on the given coordinates
        frame = original_image.crop((x, y, x + width, y + height))
        frames.append(frame)
        # Save the frame as a separate image
        frame.save(f'./suspance/result/frame_{index + i}.png')
        i = i + 1
...
# image_path = 'avatar-4.png'
# filtered_array = [arr for arr in coordinates if arr[4] == 4]
# split_image(image_path, filtered_array, 4)

# Split the image into frames

for x in range(6):
    image_path = "./suspance/suspance_reel_" + str(x) + ".png"
    filtered_array = [arr for arr in coordinates if arr[4] == x]
    split_image(image_path, filtered_array, x * 10)